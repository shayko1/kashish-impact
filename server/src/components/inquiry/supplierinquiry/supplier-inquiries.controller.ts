...
import { SupplierInquiry } from './supplierinquiry.entity';
  ...
  export class SupplierInquiriesController {
    ...
    @Patch(':id')
    async editUser(@Body() inquiry: SupplierInquiry, @Param('id') id: number): Promise<SupplierInquiry> {
      const noteEdited = await this.suplierInquiriesService.editSupplierInquiry(id, inquiry);
      return noteEdited;
    }
