...
import { ConsumerInquiry } from './consumerinquiry.entity';
  ...
  export class ConsumerInquiriesController {
    ...
    @Patch(':id')
    async editUser(@Body() inquiry: ConsumerInquiry, @Param('id') id: number): Promise<ConsumerInquiry> {
      const noteEdited = await this.consumerInquiriesService.editConsumerInquiry(id, inquiry);
      return noteEdited;
    }
